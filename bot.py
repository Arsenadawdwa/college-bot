...
ADMIN_ID = 5920598476  # обязательно int
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data_cb.startswith("score_"):
        ...
        if user_id in data["user_ratings"].get(name, {}):
            old = data["user_ratings"][name][user_id]
    ...
    # Пагинация админа
    if data_cb.startswith("admin_page_"):
        if update.effective_user.id != ADMIN_ID:
            await query.message.edit_text("❌ Нет доступа")
            return
...
async def show_admin_page(update, context):
    ...
    keyboard_row = []
    if start > 0:
        keyboard_row.append(InlineKeyboardButton("◀️ Назад", callback_data=f"admin_page_{page-1}"))
    if end < total:
        keyboard_row.append(InlineKeyboardButton("Вперёд ▶️", callback_data=f"admin_page_{page+1}"))
    keyboard = []
    if keyboard_row:
